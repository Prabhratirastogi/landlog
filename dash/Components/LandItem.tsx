...
export const LandItem = ({ name, location, count }: LandInfo) => {
  return (
    <div className='flex p-4 hover:ring-1 ring-inset ring-blue-200/30 bg-blue-100/40 hover:bg-blue-200/40 hover:shadow-inner'>
      ...
      <div className='flex flex-col flex-grow'>
        <span className='capitalize text-sky-600'>{ name }</span>
        <span className='capitalize text-xs text-slate-400'>{ location.city }, {location.state}</span>
      </div>
      <span className='place-self-center text-sky-400 bg-slate-200 rounded-full p-2 text-xs shadow-inner'>
        {count}
      </span>
    </div>
  );
